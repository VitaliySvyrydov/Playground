  ...
  */
  if (!class_exists('JestroCore')) {
    class JestroCore {
      ...
      function adminPage () {
        // global $themename, $shortname, $options;
        if ( $_REQUEST['saved'] ) echo '<div id="message" class="updated fade"><p><strong>' . $this->themename . __(' settings saved!', 'punchcut') . '</strong></p></div>';
        if ( $_REQUEST['reset'] ) echo '<div id="message" class="updated fade"><p><strong>' . $this->themename . __(' settings reset.', 'punchcut') . '</strong></p></div>'; ?>
        
<div id="v-options">
  <div class="v-top clear">
    <h1 class="v-trial"><?php echo $this->themename; ?> <?php _e('Options', 'punchcut') ?></h1>
    <div class="v-logo"><a href="http://thethemefoundry.com/">The Theme Foundry</a></div>
  </div>
  <div id="vop-header"><p><strong><?php _e( 'Need help?', 'punchcut') ?></strong> <a href="http://thethemefoundry.com/downloads/punchcut/"><?php _e( 'Read the tutorials' , 'punchcut')?></a> <?php _e('or visit the <a href="http://thethemefoundry.com/forums/">support forums.</a>', 'punchcut')?></p></div>  
  <div class="v-notice">
    <h3><?php _e( 'Free Trial', 'punchcut') ?></h3>
    <p><?php _e( 'You are using the free trial version of Punchcut. Feel free to test it for as long as you wish. Upgrade to Punchcut PRO for extra features, unlimited access to dedicated support, priority theme updates, and comprehensive theme tutorials.', 'punchcut')?></p>
    <p><a href="http://thethemefoundry.com/punchcut/"><?php _e( 'Learn more about Punchcut PRO &rarr;', 'punchcut') ?></a></p>
  </div>
  <div id="vop-body">
    <form method="post">
<?php
    ...
    <div class="v-save-button submit">
      ...
      <input class="button-primary" type="submit" value="<?php _e('Save changes', 'punchcut') ?>" name="save"/>
...
<div class="v-option">
  ...
  <div class="v-option-body clear">
    ...
    <div class="v-field colorpicker clear <?php if($pro == 'true') echo 'pro' ?>">
      ...
      <div class="v-field-d"><span><?php echo $this->options[$i]["desc"]; ?></span></div>
      <label for="<?php echo $this->options[$i]["id"]; ?>"><?php echo $this->options[$i]["name"]; ?><?php if ($pro != 'true') { ?> <a href="javascript:return false;" onclick="toggleColorpicker (this, '<?php echo $this->options[$i]["id"]; ?>', 'open', '<?php _e('show color picker', 'punchcut'); ?>', '<?php _e('hide color picker', 'punchcut'); ?>')"><?php _e('show color picker', 'punchcut'); ?><?php } ?></a></label>
    ...
    <div class="v-field textarea clear <?php if($pro == 'true') echo 'pro' ?>">
      ...
      ?>
          <div class="v-save-button submit">
            <input type="submit" value="<?php _e('Save changes', 'punchcut') ?>" name="save"/>
          </div><!--end v-save-button-->
        </div>
      </div>
      <div class="v-saveall-button submit">
        <input class="button-primary" type="submit" value="<?php _e('Save all changes', 'punchcut') ?>" name="save"/>
      </div>
      </form>
      <div class="v-reset-button submit">
        <form method="post">
          <input type="hidden" name="action" value="reset" />
          <input class="v-reset" type="submit" value="<?php _e('Reset all options', 'punchcut') ?>" name="reset"/>
        </form>
      </div>
