...
function woothemes_add_admin() {
    ...
    add_submenu_page('woothemes', $themename, 'Настройки темы', 8, 'woothemes','woothemes_page'); // Default
    add_submenu_page('woothemes', 'Available WooThemes', 'Купить темы', 8, 'woothemes_themes', 'woothemes_more_themes_page');  
}
...
?><div class="wrap" id="woo_options">
    <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post"  enctype="multipart/form-data">
        <div id="scrollme"><p class="submit"><input name="save" type="submit" value="Сохранить изменения" /></p></div>
        <div class="icon32" id="woo-icon">&nbsp;</div>
        <h2>Настройки <?php echo $themename; ?> <?php echo $update_message; ?></h2>
        <div class="info"><strong>Не можете разобраться с настройками?</strong> <a href="<?php echo $manualurl; ?>" target="_blank">Ознакомьтесь с документацией</a> или <a href="http://forum.woothemes.com" target="blank">Посетите форум поддержки</a></div>    
        
        <?php if ( $_REQUEST['saved'] ) { ?><div style="clear:both;height:20px;"></div><div class="happy"><?php echo $themename; ?>' настройки обновлены!</div><?php } ?>
        <?php if ( $_REQUEST['reset'] ) { ?><div style="clear:both;height:20px;"></div><div class="warning"><?php echo $themename; ?> настройки были сброшены!</div><?php } ?>    
    ...
    <form action="<?php echo wp_specialchars( $_SERVER['REQUEST_URI'] ) ?>" method="post">
        ...
        <input name="reset" type="submit" value="Сбросить настройки" class="reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
...
function woothemes_machine($options,$page) {
    ...
    foreach ($options as $value) {
        ...
        case "heading":
            ...
            $output .= '<p class="submit"><input name="save" type="submit" value="Сохранить изменения" /></p>';
...
function woothemes_uploader_function($id,$std){
    ...
    $uploader .= '<span class="submit"><input name="save" type="submit" value="Загрузить" class="button upload_save" /></span>';
